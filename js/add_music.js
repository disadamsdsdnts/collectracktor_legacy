				...
				if(queryResult[0][0] == "search"){
					...
					for (var actual in queryResult[0]){
						var url = checkUndefined(queryResult[0][actual]['url']),
							title = checkUndefined(queryResult[0][actual]['title']),
							artist = checkUndefined(queryResult[0][actual]['artist']),
							date = checkUndefined(queryResult[0][actual]['date']),
							format = checkUndefined(queryResult[0][actual]['format']),
							catalog = checkUndefined(queryResult[0][actual]['catalog']),
							barcode = checkUndefined(queryResult[0][actual]['barcode']),
							record = checkUndefined(queryResult[0][actual]['record']);

						url = url + '&record=' + record;

						showResults = showResults + '<a href="' + url + '" class="searchEntry"> ' + '<div class="alert alert-info"> <span class="font-italic">' +  title + '</span> <span> ( ' + date + ' )</span> <br> <span class="font-weight-bold"> ' + artist + ' </span> <br> <span> ' + format + ' </span> <br> <span> ' + catalog + ' - ' + barcode + '</span> </div>' + ' </a>';
